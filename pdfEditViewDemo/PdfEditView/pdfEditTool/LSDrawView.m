...
- (NSString *)save
{
    ...
    //linyl
    NSData *data = UIImageJPEGRepresentation(getImage, 1);
    NSString *pdfname = [NSString stringWithFormat:@"111photoToPDF%@.pdf",[self secondTime]];
    NSString *path = [WQPDFManager WQCreatePDFFileWithSrc:data toDestFile:pdfname imageWidth:[UIScreen mainScreen].bounds.size.width imageHeight:[UIScreen mainScreen].bounds.size.height-64 withPassword:nil];
    NSLog(@"合成的pdf路径是:%@",path);
    return path;
}
...
//    if (self.scale != 0) {
//        _bgImgView.transform = CGAffineTransformScale(_bgImgView.transform, self.scale, self.scale);
////        _composeView.transform = CGAffineTransformScale(_composeView.transform, self.scale, self.scale);
////        _canvasView.transform = CGAffineTransformScale(_canvasView.transform, self.scale, self.scale);
//        self.transform = CGAffineTransformScale(_canvasView.transform, self.scale, self.scale);
//    }
//    _composeView.frame = self.bounds;
//    _canvasView.frame = self.bounds;
//
...
//    }
//    if (!CGRectIsNull(self.bcImageViewframe)) {
//        _bgImgView.frame = self.bcImageViewframe;
//    }
//    CGRect fm = _bgImgView.frame;
//    fm.origin.y = 0;
//    _bgImgView.frame = fm;
